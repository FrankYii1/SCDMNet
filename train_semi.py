...
""""
创建argparse模块，它可以用于解析命令行参数和选项的标准模块
"""""
...
def main():
    ...
    train_loader_sup, train_loader_unsup, val_loader, test_loader = get_loader(cfg, seed=seed)
    ...
    optimizer_start = get_optimizer(params_list, cfg_optim)         # 产生优化器
    lr_scheduler = get_scheduler(                                   # 产生学习率衰减程序
...
def train(
    ...
    global prototype
    ema_decay_origin = cfg["net"]["ema_decay"]  # ema_decay_origin = 0.99
    ...
    for step in range(len(loader_l)):
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 1):
            ...
            logits_u_aug, label_u_aug = torch.max(pred_u_teacher, dim=1)  #返回教师模型预测的第一维的最大值，且返回索引

            if np.random.uniform(0, 1) < 0.5 and cfg["trainer"]["unsupervised"].get(   # np.random.uniform(low, high):从一个均匀分布[low,high)中随机采样
                "apply_aug", False
            ):
            ...
            drop_percent = cfg["trainer"]["unsupervised"].get("drop_percent", 100)  # 80
        ...
        if i_iter % 10 == 0 and rank == 0:
            ...
            tb_logger.add_scalar("lr", learning_rates.val, epoch)
            tb_logger.add_scalar("Sup Loss", sup_losses.val, epoch)
            tb_logger.add_scalar("Uns Loss", uns_losses.val, epoch)
            tb_logger.add_scalar("Con Loss", con_losses.val, epoch)
        return sup_losses.val, uns_losses.val, con_losses.val
