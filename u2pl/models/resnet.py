...
#from attention.SE_Net import SELayer
from tricks.attention import SELayer, ChannelAttention, SpatialAttention, Cbam, Eca_Model, NonLocalBlockND
...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    ...
    return nn.Conv2d(
        ...
        padding=dilation,
        groups=groups,
        bias=False,   # 使用bn时，不需要使用bias
        dilation=dilation,
    )


def conv1x1(in_planes, out_planes, stride=1):
    """1x1 convolution， 用来降维和升维"""
...
class BasicBlock(nn.Module):
    """
    用于ResNet18和34的残差结构
    expansion设为1，表示它们的残差结构中，通道数保持不变
    """
    expansion = 1
