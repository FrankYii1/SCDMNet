...
class Compose(object):
    """
    Composes several seg segtransforms together.
...
class RandResize(object):
    """
   随机调整图片的大小 & label with scale factor in [scale_min, scale_max]
    """
    ...
    def __call__(self, image, label):
        ...
        image = F.interpolate(
            image, size=(new_h, new_w), mode="bilinear", align_corners=False    #上采样，将原(h,w)上采样到(new_h, new_w)
        )
...
class RandRotate(object):
    """
    随机旋转图像 & label with rotate factor in [rotate_min, rotate_max]
    """
