...
class ISPRS_dset(BaseDataset):
    def __init__(self, data_root, data_list, trs_form, seed, n_sup, split="val"):
        super(ISPRS_dset, self).__init__(data_list)
...
def build_ISPRSloader(split, all_cfg, seed=0):
    ...
    n_sup = cfg.get("n_sup", 1296)

    # build transform
    trs_form = build_transfrom(cfg)
    dset = ISPRS_dset(cfg["data_root"], cfg["data_list"], trs_form, seed, n_sup, split)
...
def build_ISPRS_semi_loader(split, all_cfg, seed=0):
