...
from .ISPRS import build_ISPRS_semi_loader, build_ISPRSloader
...
def get_loader(cfg, seed=0):
    ...
    elif cfg_dataset["type"] == "ISPRS_semi":
        train_loader_sup, train_loader_unsup = build_ISPRS_semi_loader(
            "train", cfg, seed=seed
        )
        val_loader = build_ISPRS_semi_loader("val", cfg)
        test_loader = build_ISPRS_semi_loader('test', cfg)
        logger.info("Get loader Done...")
        return train_loader_sup, train_loader_unsup, val_loader, test_loader

    elif cfg_dataset["type"] == "ISPRS":
        train_loader_sup = build_ISPRSloader("train", cfg, seed=seed)
        val_loader = build_ISPRSloader("val", cfg)
        test_loader = build_ISPRS_semi_loader('test', cfg)
        logger.info("Get loader Done...")
        return train_loader_sup, val_loader, test_loader
